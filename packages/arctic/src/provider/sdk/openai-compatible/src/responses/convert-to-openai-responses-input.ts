...
export async function convertToOpenAIResponsesInput({
  ...
  systemMessageMode: "system" | "developer" | "remove" | "extracted"
  ...
  const instructions = formatInstructions(instructionParts)

  return { input, warnings, instructions }
}

function formatInstructions(parts: string[]): string | undefined {
  if (parts.length === 0) return undefined
  const combined = parts.join("\n\n").trim()
  if (!combined) return undefined
  if (combined.includes("<INSTRUCTIONS>")) {
    return combined
  }

  const projectLabel = process.env.ARCTIC_PROJECT_NAME ?? deriveDefaultProjectLabel()
  return `# AGENTS.md instructions for ${projectLabel}\n\n<INSTRUCTIONS>\n${combined}\n</INSTRUCTIONS>`
}

function deriveDefaultProjectLabel(): string {
  try {
