...
export class OpenAIResponsesLanguageModel implements LanguageModelV2 {
  ...
  private async getArgs({
    ...
    const combinedInstructions = openaiOptions?.instructions ?? generatedInstructions
    ...
    for (const part of parts) {
      switch (part.type) {
        ...
        case "finish":
          ...
          usage = part.usage
          Object.assign(providerMetadata, part.providerMetadata)
          break
        case "tool-call":
          toolCalls.push({
            ...
            if (isResponseOutputItemAddedChunk(value)) {
              ...
              if (value.item.type === "function_call") {
                ...
                controller.enqueue({
                  ...
                  toolName: "code_interpreter",
                  result: {
                    outputs: value.item.outputs as z.infer<typeof codeInterpreterOutputSchema>["outputs"],
...
function isResponseOutputItemDoneChunk(
  ...
  return (
    ...
    typeof (chunk as Record<string, unknown>).item === "object" &&
    chunk.item !== null &&
    typeof (chunk as Record<string, unknown>).output_index === "number"
  )
}
...
type ResponsesModelConfig = {
  ...
  systemMessageMode: "remove" | "system" | "developer" | "extracted"
...
function getResponsesModelConfig(modelId: string): ResponsesModelConfig {
  ...
  if (
    ...
    // Codex models expect instructions in the 'instructions' field, not as a message
    if (modelId.includes("codex")) {
      return {
        ...defaults,
        isReasoningModel: true,
        systemMessageMode: "extracted",
      }
    }
