...
export async function convertSseToJson(response: Response, headers: Headers): Promise<Response> {
	if (!response.body) {
		throw new Error('[arctic-codex-plugin] Response has no body');
	}
	...
	try {
		...
		if (!finalResponse) {
			console.error('[arctic-codex-plugin] Could not find final response in SSE stream');
		...
		console.error('[arctic-codex-plugin] Error converting stream:', error);
