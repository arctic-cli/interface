...
import { getArcticCodexPrompt } from "../prompts/opencode-codex.js";
 ...
 *
 * @param input - Original input array from Arctic/AI SDK
...
/**
 * Check if an input item is the Arctic system prompt
 * Uses cached Arctic codex.txt for verification with fallback to text matching
 * @param item - Input item to check
 * @param cachedPrompt - Cached Arctic codex.txt content
 * @returns True if this is the Arctic system prompt
 */
export function isArcticSystemPrompt(
	item: InputItem,
	...
	// Primary check: Compare against cached Arctic prompt
	if (cachedPrompt) {
	...
	// Fallback check: Known Arctic prompt signature (for safety)
	// This catches the prompt even if cache fails
	return contentText.startsWith("You are a coding agent running in");
}

/**
 * Filter out Arctic system prompts from input
 * Used in CODEX_MODE to replace Arctic prompts with Codex-Arctic bridge
 * @param input - Input array
 * @returns Input array without Arctic system prompts
 */
export async function filterArcticSystemPrompts(
	input: InputItem[] | undefined,
): Promise<InputItem[] | undefined> {
	if (!Array.isArray(input)) return input;

	// Fetch cached Arctic prompt for verification
	let cachedPrompt: string | null = null;
	try {
		cachedPrompt = await getArcticCodexPrompt();
	} catch {
	...
	return input.filter((item) => {
		...
		// Filter out Arctic system prompts
		return !isArcticSystemPrompt(item, cachedPrompt);
	});
}

/**
 * Add Codex-Arctic bridge message to input if tools are present
 * @param input - Input array
...
export async function transformRequestBody(
	...
	// (Arctic passes its session identifier). We no longer synthesize one here.
	...
	if (body.input && Array.isArray(body.input)) {
		...
		if (codexMode) {
			// CODEX_MODE: Remove Arctic system prompt, add bridge prompt
			body.input = await filterArcticSystemPrompts(body.input);
