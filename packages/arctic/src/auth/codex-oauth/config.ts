...
const CONFIG_PATH = join(homedir(), ".arctic", "openai-codex-auth-config.json");
...
/**
 * Load plugin configuration from ~/.arctic/openai-codex-auth-config.json
...
export function loadPluginConfig(): PluginConfig {
	try {
		...
		console.warn(
			`[arctic-codex-plugin] Failed to load config from ${CONFIG_PATH}:`,
...
export function getCodexMode(pluginConfig: PluginConfig): boolean {
	...
	if (process.env.ARCTIC_CODEX_MODE !== undefined) {
		return process.env.ARCTIC_CODEX_MODE === "1";
	}
