...
export async function exchangeAuthorizationCode(
	...
	if (!res.ok) {
		...
		console.error("[arctic-codex-plugin] code->token failed:", res.status, text);
		return { type: "failed" };
	}
	...
	if (
		...
		console.error("[arctic-codex-plugin] token response missing fields:", json);
		return { type: "failed" };
	}
	return {
		type: "success",
...
export async function refreshAccessToken(refreshToken: string): Promise<TokenResult> {
	try {
		...
		if (!response.ok) {
			...
			console.error(
				"[arctic-codex-plugin] Token refresh failed:",
				response.status,
				text,
			);
			return { type: "failed" };
		...
		if (
			...
			console.error(
				"[arctic-codex-plugin] Token refresh response missing fields:",
				json,
			);
			return { type: "failed" };
		}
		...
		console.error("[arctic-codex-plugin] Token refresh error:", err);
		return { type: "failed" };
	}
}
