...
export const RunCommand = cmd({
  ...
  handler: async (args) => {
    ...
    const execute = async (sdk: ArcticClient, sessionID: string) => {
    ...
    if (args.attach) {
      const sdk = createArcticClient({ baseUrl: args.attach })
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createArcticClient({ baseUrl: `http://${server.hostname}:${server.port}` })
