...
export const SessionForkCommand = cmd({
  command: "fork <session-id> [message-id]",
  describe: "fork a session from a specific point",
  builder: (yargs: Argv) =>
    yargs
      ...
      .positional("message-id", {
        describe: "optional message id to fork at (exclusive)",
        type: "string",
      }),
  handler: async (args) => {
    await bootstrap(process.cwd(), async () => {
      const sessionID = args["session-id"] as string
      const messageID = args["message-id"] as string | undefined
      if (!sessionID) {
        UI.error("Session id is required.")
        return
      }
