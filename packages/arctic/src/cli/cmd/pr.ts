...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run arctic",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for arctic session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/arctic\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found arctic session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`arctic import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting arctic...")
        UI.println()

        // Launch arctic TUI with session ID if available
        const { spawn } = await import("child_process")
        const arcticArgs = sessionId ? ["-s", sessionId] : []
        const arcticProcess = spawn("arctic", arcticArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          arcticProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`arctic exited with code ${code}`))
          })
          arcticProcess.on("error", reject)
        })
      },
    })
  },
