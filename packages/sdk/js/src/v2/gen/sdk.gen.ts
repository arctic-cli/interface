...
import type {
  ...
  SessionBenchmarkApplyErrors,
  SessionBenchmarkApplyResponses,
  SessionBenchmarkStartErrors,
  SessionBenchmarkStartResponses,
  SessionBenchmarkStopErrors,
  SessionBenchmarkStopResponses,
  SessionBenchmarkUndoErrors,
  SessionBenchmarkUndoResponses,
...
class HeyApiRegistry<T> {
  ...
  get(key?: string): T {
    ...
    if (!instance) {
      throw new Error(`No SDK client found. Create one with "new ArcticClient()" to fix this error.`)
    }
    return instance
  }
...
export class Benchmark extends HeyApiClient {
   ...
   *
   * Enable benchmark mode on a session and create child sessions for model comparison.
   */
   ...
   * Stop benchmark mode
   *
   * Disable benchmark mode for a session and clear benchmark metadata.
   */
  ...
  /**
   * Apply benchmark changes
   *
   * Apply the changes from a benchmark child session to the working tree.
   */
   ...
   *
   * Restore the working tree to the benchmark base snapshot.
   */
  public undo<ThrowOnError extends boolean = false>(
    parameters: {
      sessionID: string
      directory?: string
      allowDirty?: boolean
...
export class Usage extends HeyApiClient {
   ...
   * Get provider usage
   *
   * Fetch usage information for configured providers.
   */
  public providers<ThrowOnError extends boolean = false>(
    parameters?: {
      directory?: string
...
export class ArcticClient extends HeyApiClient {
  public static readonly __registry = new HeyApiRegistry<ArcticClient>()

  constructor(args?: { client?: Client; key?: string }) {
    super(args)
    ArcticClient.__registry.set(this, args?.key)
  }
